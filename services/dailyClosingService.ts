...
export const dailyClosingService = {
  async getByDate(businessDate: string): Promise<DailyClosingServiceResult> {
    ...
    try {
      ...
      const result = await supabase.from("daily_closings").select("*").eq("business_date", businessDate).maybeSingle();
  ...
  async getPaymentMethods(dailyClosingId: string): Promise<DailyClosingServiceResult<DailyClosingPaymentMethod[]>> {
    ...
    try {
      ...
      const result = await supabase.from("daily_closing_payment_methods").select("id,daily_closing_id,payment_method,expected_amount,actual_amount,discrepancy,transaction_count,notes").eq("daily_closing_id", dailyClosingId).order("payment_method");
  ...
  async open(businessDate: string, initialOpeningCash?: number | null): Promise<DailyClosingServiceResult> {
    ...
    return callRpc("open daily closing", async () => {
      const result = await supabase.rpc("open_daily_closing", { p_business_date: businessDate, p_initial_opening_cash: initialOpeningCash ?? null });
  ...
  async reconcile(dailyClosingId: string): Promise<DailyClosingServiceResult> {
    ...
    return callRpc("reconcile daily closing", async () => {
      const result = await supabase.rpc("reconcile_daily_closing", { p_daily_closing_id: dailyClosingId });
      return { data: Array.isArray(result.data) ? result.data[0] ?? null : result.data, error: result.error };
    });
  },

  async close(dailyClosingId: string, actualCash: number, notes?: string | null, paymentMethodActuals?: PaymentMethodActuals): Promise<DailyClosingServiceResult> {
    if (!dailyClosingId.trim()) return validation("Daily closing ID is required.");
    ...
    return callRpc("close daily closing", async () => {
      const result = paymentMethodActuals
        ? await supabase.rpc("close_daily_closing_with_payment_methods", { p_daily_closing_id: dailyClosingId, p_actual_cash: actualCash, p_notes: notes?.trim() || null, p_payment_methods: paymentMethodActuals })
  ...
  async reopen(dailyClosingId: string, reason: string): Promise<DailyClosingServiceResult> {
    ...
    return callRpc("reopen daily closing", async () => {
      const result = await supabase.rpc("reopen_daily_closing", { p_daily_closing_id: dailyClosingId, p_reason: normalizedReason });
      return { data: Array.isArray(result.data) ? result.data[0] ?? null : result.data, error: result.error };
    });
  },
};
