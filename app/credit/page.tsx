...
export default function CreditPage() {
  ...
  function remind(p: Payable) {
    ...
    if (openWhatsApp(p.phone, `Hello ${p.person_name}, this is Danchrista Four Communication. This is a reminder that we currently owe you ${money(Number(p.balance))}.${due} Thank you.`)) setMessage("WhatsApp opened with the reminder ready to send.");
  }

  return <AppLayout><main className="space-y-6">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#8A641D]">Credit book</p><h1 className="mt-1 font-heading text-3xl font-bold tracking-tight text-[#102A43]">People Danchrista Owes</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-[#62788F]">Goods or value we collected from someone and still need to pay. Credit stays separate from people who owe Danchrista.</p></div><Link href="/outstanding" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-[#CFE3F2] bg-white px-5 text-sm font-bold text-[#183b34]">People who owe us</Link></header>
    ...
    <section className="rounded-2xl border border-[#7FE7D6]/35 bg-[#fffaf0] p-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#8A641D]">New credit account</p><h2 className="mt-1 font-heading text-xl font-bold text-[#102A43]">Record goods collected</h2></div><form onSubmit={createPayable} className="mt-5 space-y-4">
      <div className="flex gap-2"><button type="button" onClick={() => setPersonMode("existing")} className={`rounded-lg px-3 py-2 text-xs font-bold ${personMode === "existing" ? "bg-[#0B3D91] text-white" : "border bg-white text-slate-600"}`}>Existing person</button><button type="button" onClick={() => { setPersonMode("new"); setCustomerId(""); }} className={`rounded-lg px-3 py-2 text-xs font-bold ${personMode === "new" ? "bg-[#0B3D91] text-white" : "border bg-white text-slate-600"}`}>New person</button></div>
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {personMode === "existing" ? <label className="text-xs font-bold text-[#536B83]">Person<select value={customerId} onChange={e => choosePerson(e.target.value)} className="mt-1 h-11 w-full rounded-xl border border-[#CFE3F2] bg-white px-3 text-sm font-normal"><option value="">Select person</option>{people.map(p => <option key={p.id} value={p.id}>{p.full_name}{p.phone ? ` · ${p.phone}` : ""}</option>)}</select></label> : <Field label="Person" value={person} onChange={setPerson} placeholder="Name" />}
        ...
        <label className="text-xs font-bold text-[#536B83]">Due date<input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="mt-1 h-11 w-full rounded-xl border border-[#CFE3F2] bg-white px-3 text-sm font-normal" /></label>
        <Field label="Note" value={notes} onChange={setNotes} placeholder="Optional" />
        <button disabled={saving || (personMode === "existing" && !customerId)} className="mt-5 h-11 rounded-xl bg-[#0B3D91] px-4 text-sm font-bold text-white disabled:opacity-50">{saving ? "Saving..." : "Add credit"}</button>
      </div>
    </form></section>
    {message && <div className="rounded-xl border border-[#CFE3F2] bg-white px-4 py-3 text-sm font-medium text-[#536B83]">{message}</div>}
    <section className="overflow-hidden rounded-2xl border border-[#CFE3F2] bg-white"><div className="flex flex-col gap-3 border-b border-[#edf0ed] p-5 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#0B3D91]">Accounts</p><h2 className="mt-1 font-heading text-lg font-bold">People we owe</h2></div><div className="flex flex-col gap-2 sm:flex-row"><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, phone or goods" className="h-10 rounded-xl border border-[#CFE3F2] px-3 text-sm" /><select value={filter} onChange={e => setFilter(e.target.value)} className="h-10 rounded-xl border border-[#CFE3F2] bg-white px-3 text-sm"><option value="open">All active</option><option value="overdue">Overdue</option><option value="due">Due soon</option><option value="paid">Paid</option><option value="all">All records</option></select></div></div><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead className="bg-[#F4FAFF]"><tr><th className="px-5 py-3">Person</th><th className="px-5 py-3">Goods</th><th className="px-5 py-3">Agreed</th><th className="px-5 py-3">Paid</th><th className="px-5 py-3">Balance</th><th className="px-5 py-3">Due</th><th className="px-5 py-3">Status</th><th className="px-5 py-3"></th></tr></thead><tbody>{visible.map(p => { const status = statusOf(p); return <tr key={p.id} className="border-t border-[#edf0ed]"><td className="px-5 py-4"><button onClick={() => void loadAccount(p)} className="text-left font-bold text-[#183b34] hover:underline">{p.person_name}</button><div className="text-[11px] text-[#8b918e]">{p.phone || "No number"}</div></td><td className="px-5 py-4 text-[#536B83]">{p.description || "—"}</td><td className="px-5 py-4">{money(Number(p.agreed_amount))}</td><td className="px-5 py-4 text-emerald-700">{money(Number(p.amount_paid))}</td><td className="px-5 py-4 font-bold text-red-700">{money(Number(p.balance))}</td><td className="px-5 py-4 text-xs">{p.due_date ? new Date(`${p.due_date}T00:00:00`).toLocaleDateString("en-NG") : "No date"}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${status.className}`}>{status.label}</span></td><td className="px-5 py-4"><div className="flex gap-2"><button onClick={() => void loadAccount(p)} className="rounded-lg border border-[#CFE3F2] px-3 py-2 text-xs font-bold">Open</button>{Number(p.balance) > 0.01 && p.phone && <button onClick={() => remind(p)} className="rounded-lg bg-[#e9f5ef] px-3 py-2 text-xs font-bold text-[#176447]">WhatsApp</button>}</div></td></tr>; })}</tbody></table>{visible.length === 0 && <div className="py-12 text-center text-sm text-[#62788F]">No credit accounts match this view.</div>}</div></section>
    {selected && <div className="fixed inset-0 z-50 overflow-y-auto bg-black/30 p-4"><div className="mx-auto my-8 w-full max-w-4xl rounded-2xl bg-white shadow-2xl"><div className="flex items-start justify-between border-b border-[#edf0ed] p-6"><div><h2 className="font-heading text-2xl font-bold text-[#102A43]">{selected.person_name}</h2><p className="text-sm text-[#62788F]">{selected.phone || "No phone"} · Danchrista owes {money(Number(selected.balance))}</p></div><button onClick={() => setSelected(null)} className="rounded-lg border px-3 py-2 text-sm font-bold">Close</button></div><div className="grid gap-5 p-6 lg:grid-cols-2"><section><h3 className="font-bold">Goods collected</h3><div className="mt-3 space-y-2">{items.map(i => <div key={i.id} className="flex justify-between rounded-xl bg-[#F4FAFF] p-3 text-sm"><span>{i.item_name} × {i.quantity}</span><b>{money(i.quantity * i.unit_value)}</b></div>)}</div><form onSubmit={addItem} className="mt-4 grid gap-2 sm:grid-cols-3"><input value={addItemName} onChange={e => setAddItemName(e.target.value)} placeholder="Another item" className="h-10 rounded-lg border px-3 text-sm" /><input value={addItemQty} onChange={e => setAddItemQty(e.target.value)} type="number" min="1" placeholder="Qty" className="h-10 rounded-lg border px-3 text-sm" /><input value={addItemValue} onChange={e => setAddItemValue(e.target.value)} type="number" min="1" placeholder="Value" className="h-10 rounded-lg border px-3 text-sm sm:col-span-2" /><button disabled={saving} className="h-10 rounded-lg bg-[#0B3D91] text-xs font-bold text-white">Add item</button></form></section><section><h3 className="font-bold">Payments</h3><form onSubmit={recordPayment} className="mt-3 flex gap-2"><input value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} type="number" min="1" max={selected.balance} placeholder="Amount" className="h-10 min-w-0 flex-1 rounded-lg border px-3 text-sm" /><select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} className="h-10 rounded-lg border bg-white px-2 text-sm"><option value="cash">Cash</option><option value="transfer">Transfer</option><option value="pos">POS</option><option value="other">Other</option></select><button disabled={saving || Number(selected.balance) <= 0} className="h-10 rounded-lg bg-[#0B3D91] px-3 text-xs font-bold text-white">Pay</button></form><div className="mt-4 space-y-2">{payments.map(p => <div key={p.id} className="flex justify-between rounded-xl border p-3 text-sm"><span>{new Date(p.paid_at).toLocaleDateString("en-NG")} · {p.payment_method}</span><b>{money(Number(p.amount))}</b></div>)}</div></section></div></div></div>}
  </main></AppLayout>;
}

function Field({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; type?: string }) { return <label className="text-xs font-bold text-[#536B83]">{label}<input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="mt-1 h-11 w-full rounded-xl border border-[#CFE3F2] bg-white px-3 text-sm font-normal" /></label>; }
function Metric({ label, value, tone }: { label: string; value: string; tone?: "red" | "amber" }) { return <div className={`rounded-2xl border bg-white p-5 ${tone === "red" ? "border-red-100" : tone === "amber" ? "border-amber-100" : "border-[#CFE3F2]"}`}><p className="text-xs font-bold uppercase tracking-wider text-[#62788F]">{label}</p><p className="mt-2 text-2xl font-bold text-[#102A43]">{value}</p></div>; }
