...
export async function POST(request:Request){
 try{
  ...
  if(body.type==="sale"){
   ...
   message=["DANCHRISTA FOUR COMMUNICATION","🧾 New sale recorded",`Amount: ₦${Number(row.total||0).toLocaleString("en-NG")}`,`Customer: ${customer||"Walk-in"}`,`Payment: ${row.payment_method||"Not specified"}`,`Staff: ${row.staff_name||"Not specified"}`,`Sale: ${row.id}`].join("\n");
  }else{
   ...
   message=["DANCHRISTA FOUR COMMUNICATION","🔧 Repair update",`Device: ${deviceName}`,`Customer: ${customer||"Walk-in"}`,`Problem: ${row.issue||"Not specified"}`,`Status: ${row.status||"Not specified"}`,`Technician: ${row.technician||"Not assigned"}`,`Estimated: ₦${Number(row.estimated_cost||0).toLocaleString("en-NG")}`,`Final: ₦${Number(row.final_cost||0).toLocaleString("en-NG")}`,`Repair: ${row.id}`].join("\n");
  }
