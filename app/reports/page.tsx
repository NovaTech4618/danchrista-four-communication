...
export default function ReportsPage() {
  ...
  return <AppLayout><div className="space-y-7">
    <header className="flex flex-wrap items-end justify-between gap-4">
      <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#0B3D91]">Danchrista owner records</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-950">Reports</h1><p className="mt-1 text-sm text-slate-500">Sales, repairs, stock, money received, costs and what is still owed.</p></div>
    ...
    {loading ? <div className="rounded-2xl border bg-white p-10 text-center text-sm text-slate-500">Loading records…</div> : <>
      ...
      <ProfitLossBreakdown from={range.start} to={range.end}/><SalesAnalytics from={range.start} to={range.end}/><RepairTurnaroundAnalytics from={range.start} to={range.end}/><CustomerAnalytics from={range.start} to={range.end}/><StockValuation/><TechnicianPerformance/>
    </>}
  </div></AppLayout>;
}
...
function Mix({ title, value, total }: { title: string; value: number; total: number }) { const pct = total > 0 ? Math.min(100, value / total * 100) : 0; return <div><div className="mb-1 flex justify-between text-xs"><span className="text-slate-600">{title}</span><strong>{money(value)}</strong></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#0B3D91]" style={{ width: `${pct}%` }}/></div></div>; }
