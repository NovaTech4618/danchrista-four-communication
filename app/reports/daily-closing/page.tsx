...
export default function DailyClosingPage() {
  ...
  const load = useCallback(async (date: string, reconcileOpen = true) => {
    ...
    if (current?.status === "open" && reconcileOpen) {
      ...
      if (reconciled.error) setError(reconciled.error.message);
    ...
    <AppLayout>
      <div className="space-y-5 pb-10">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500"><span className="rounded-full bg-slate-100 px-3 py-1.5 font-semibold">Company timezone: {timezone}</span></div>
        </header>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Button variant="outline" disabled={loading || working || !businessDate} onClick={() => void load(businessDate)}><RefreshCw className={working ? "animate-spin" : ""} /> Refresh reconciliation</Button>
          </CardContent>
        </Card>

        {loading ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[1, 2, 3, 4].map((item) => <div key={item} className="h-28 animate-pulse rounded-2xl bg-slate-100" />)}</div>
        ) : !closing ? (
          <Card><CardHeader><CardTitle>Open this business day</CardTitle><CardDescription>No closing exists for {businessDate}. Opening cash must be entered explicitly; it will never be assumed to be ₦0.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="max-w-sm"><label htmlFor="opening-cash" className="mb-1.5 block text-sm font-semibold text-slate-700">Initial opening cash</label><Input id="opening-cash" type="number" min="0" step="0.01" inputMode="decimal" placeholder="0.00" value={openingCashInput} onChange={(event) => setOpeningCashInput(event.target.value)} /></div><Button disabled={working} onClick={() => void handleOpen()}>{working ? "Opening…" : "Open daily closing"}</Button></CardContent></Card>
        ) : (
          <>
