...
export default function SearchPage() {
  ...
  return <AppLayout><div className="mx-auto w-full max-w-5xl space-y-6">
    <header>
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#1d6a54]">Find anything</p>
      <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-950">Search Amezing Limited</h1>
      <p className="mt-1 text-sm leading-6 text-slate-500">Find customers, devices, repairs, stock, invoices and purchases.</p>
    </header>
    ...
    <div className="sticky top-[72px] z-20 rounded-xl border border-slate-200 bg-white p-2 shadow-sm">
      <div className="relative flex items-center">
        ...
        <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Escape") setQ(""); }} placeholder="Name, phone, IMEI, SKU, repair, invoice or supplier…" aria-label="Search Amezing Limited records" className="h-12 w-full rounded-lg border-0 bg-slate-50 pl-11 pr-11 text-sm outline-none ring-1 ring-inset ring-slate-200 focus:bg-white focus:ring-2 focus:ring-[#1d6a54]" />
      ...
      <div className="hidden px-2 pt-2 text-[11px] text-slate-400 sm:flex sm:justify-between"><span>Searches Amezing Limited business records</span><span>Esc to clear</span></div>
    </div>

    {loading && <div className="flex items-center gap-2 text-sm text-slate-500"><span className="size-2 animate-pulse rounded-full bg-[#1d6a54]" />Searching…</div>}
