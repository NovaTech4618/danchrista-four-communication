...
const headers = ["item_name","category","brand","compatible_models","sku","selling_price","minimum_selling_price","cost_price","quantity","minimum_stock","supplier","shelf_location","notes","image_url"];
...
export default function InventoryImportPage() {
  ...
  async function importCsv() {
    ...
    try { const text=await file.text(); const rows=parseCsv(text); if(rows.length<2) throw new Error("CSV must contain a header row and at least one item."); const source=rows[0].map(x=>x.toLowerCase()); const missing=headers.filter(h=>!source.includes(h) && h!=="image_url"); if(missing.length) throw new Error(`Missing columns: ${missing.join(", ")}`);
      ...
      const records=rows.slice(1).map(r=>{const get=(h:string)=>r[source.indexOf(h)]??""; const sellingPrice=Number(get("selling_price")||0); const minimumSellingPrice=Number(get("minimum_selling_price") || sellingPrice); return {company_id:profile.company_id,branch_id:profile.branch_id??null,item_name:get("item_name"),category:get("category")||"Part",brand:get("brand")||null,compatible_models:get("compatible_models")||null,sku:get("sku")||null,selling_price:sellingPrice,minimum_selling_price:minimumSellingPrice,cost_price:get("cost_price")?Number(get("cost_price")):null,quantity:Number(get("quantity")||0),minimum_stock:Number(get("minimum_stock")||5),supplier:get("supplier")||null,shelf_location:get("shelf_location")||null,notes:get("notes")||null,image_url:get("image_url")||null};}).filter(x=>x.item_name);
