      ...
      <main className="mx-auto w-full max-w-[1500px] space-y-6">
        ...
        <section className="rounded-2xl border border-[#CFE3F2] bg-white p-4 shadow-[0_10px_28px_rgba(18,59,52,0.05)]">
          <div className="grid gap-3 lg:grid-cols-[1fr_180px_180px]">
            <div className="relative">
              <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#52677D]" />
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-xs text-[#62788F]">
            ...
            {brand && <span>Brand: <strong className="text-[#29445F]">{brand === "__other" ? "Other / unbranded" : brand}</strong></span>}
        ...
        <section className="overflow-hidden rounded-2xl border border-[#CFE3F2] bg-white shadow-[0_10px_28px_rgba(11,61,145,0.08)]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#CFE3F2] px-5 py-4">
            ...
            {category && <span className="rounded-full bg-[#EAF7FF] px-3 py-1 text-xs font-semibold text-[#0B3D91]">{category}{brand ? ` · ${brand === "__other" ? "Other / unbranded" : brand}` : ""}</span>}
          </div>
...
function ShelfCard({ title, count, description, icon: Icon, onClick }: { title:string; count:number; description:string; icon:typeof Package; onClick:()=>void }) {
  return <button type="button" onClick={onClick} className="group rounded-2xl border border-[#CFE3F2] bg-white p-5 text-left transition hover:-translate-y-0.5 hover:border-[#0B3D91] hover:shadow-md">
    <div className="flex items-start justify-between"><span className="grid size-11 place-items-center rounded-xl bg-[#EAF7FF] text-[#0B3D91]"><Icon className="size-5" /></span><ChevronRight className="size-5 text-[#52677D] transition group-hover:translate-x-1 group-hover:text-[#0B3D91]" /></div>
...
function CategoryCard({ label, count, description, icon: Icon, onClick, active }: { label:string; count:number; description:string; icon:typeof Package; onClick:()=>void; active?:boolean }) {
  return <button type="button" onClick={onClick} className={`group rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 hover:border-[#0B3D91] hover:shadow-sm ${active ? "border-[#0B3D91] bg-[#EAF7FF] ring-2 ring-[#0B3D91]/10" : "border-[#CFE3F2] bg-white"}`}>
    <div className="flex items-center justify-between"><span className="grid size-9 place-items-center rounded-lg bg-white text-[#0B3D91] ring-1 ring-[#CFE3F2]"><Icon className="size-4" /></span><ChevronRight className="size-4 text-[#9aa8a2] group-hover:text-[#0B3D91]" /></div>
