-- A sale-level discount must not silently bypass an item's minimum selling price.
-- An authorised price override is required when the effective sale total falls below
-- the combined minimum selling prices.
...
as $function$
declare v_company_id uuid; v_user uuid; v_sale uuid; v_subtotal numeric:=0; v_min_subtotal numeric:=0; v_total numeric; v_item jsonb; v_inventory_id uuid; v_qty integer; v_unit numeric; v_cost numeric; v_available integer; v_floor numeric; v_override boolean; v_has_override boolean:=false; v_branch uuid; v_sale_branch uuid;
begin
 ...
 select company_id into v_company_id from public.profiles where id=v_user; if v_company_id is null then raise exception 'Company not found'; end if;
 ...
 for v_item in select * from jsonb_array_elements(p_items) loop
  v_inventory_id:=(v_item->>'inventory_id')::uuid; v_qty:=(v_item->>'quantity')::integer; v_unit:=(v_item->>'unit_price')::numeric; v_override:=coalesce((v_item->>'price_override')::boolean,false);
  if v_qty<=0 or v_unit<0 then raise exception 'Invalid sale item'; end if;
  select quantity,branch_id,minimum_selling_price,coalesce(cost_price,0) into v_available,v_branch,v_floor,v_cost from public.inventory where id=v_inventory_id and company_id=v_company_id for update;
  ...
  v_subtotal:=v_subtotal+(v_qty*v_unit); v_min_subtotal:=v_min_subtotal+(v_qty*coalesce(v_floor,0));
 end loop;
 ...
 if v_total < v_min_subtotal and not v_has_override then raise exception 'Discount would take the sale below the minimum selling prices.'; end if;
 if v_total < v_min_subtotal and not public.has_permission('sales.price_override') then raise exception 'Authorised override is required for a discounted sale below minimum selling prices.'; end if;
 ...
 for v_item in select * from jsonb_array_elements(p_items) loop
  v_inventory_id:=(v_item->>'inventory_id')::uuid; v_qty:=(v_item->>'quantity')::integer; v_unit:=(v_item->>'unit_price')::numeric; v_override:=coalesce((v_item->>'price_override')::boolean,false);
  select coalesce(cost_price,0),minimum_selling_price into v_cost,v_floor from public.inventory where id=v_inventory_id and company_id=v_company_id;
  ...
  if v_override and v_unit<coalesce(v_floor,0) then perform public.write_audit_log('sale.price_floor_override','sale',v_sale,null,null,jsonb_build_object('inventory_id',v_inventory_id,'quantity',v_qty,'unit_price',v_unit,'minimum_selling_price',v_floor,'user_id',v_user)); end if;
 end loop;
 return v_sale;
end;
$function$;
