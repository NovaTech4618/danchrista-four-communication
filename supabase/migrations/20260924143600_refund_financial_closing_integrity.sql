-- Financial integrity: allow approved customer refunds and include them in Daily Closing net profit.
-- Refunds are real cash outflows and must not be blocked by the ledger category constraint.
-- Daily Closing keeps gross revenue visible but subtracts customer refunds when calculating net profit.
...
as $function$
declare
  v_timezone text; v_start timestamptz; v_end timestamptz; v_user_company uuid;
  v_sales numeric:=0; v_repairs numeric:=0; v_engineer_revenue numeric:=0; v_standalone_invoice numeric:=0;
  v_cash numeric:=0; v_transfer numeric:=0; v_pos numeric:=0; v_other numeric:=0; v_total_cash_received numeric:=0;
  v_customer_open numeric:=0; v_customer_charges numeric:=0; v_customer_payments numeric:=0; v_customer_close numeric:=0;
  v_engineer_open numeric:=0; v_engineer_charges numeric:=0; v_engineer_credits numeric:=0; v_engineer_payments numeric:=0; v_engineer_close numeric:=0;
  v_sale_cogs numeric:=0; v_repair_cogs numeric:=0; v_engineer_cogs numeric:=0; v_return_cogs numeric:=0;
  v_operating_expenses numeric:=0; v_engineer_direct_cost numeric:=0; v_customer_refunds numeric:=0;
  v_cash_inflows numeric:=0; v_cash_outflows numeric:=0;
  v_total_revenue numeric:=0; v_total_cogs numeric:=0; v_gross_profit numeric:=0; v_net_profit numeric:=0;
begin
  ...
  from public.sales s
  where s.company_id=p_company_id and s.sale_date>=v_start and s.sale_date<v_end;
  ...
  from public.engineer_transactions t
  where t.company_id=p_company_id
    and t.transaction_type in ('parts_out','service_charge','parts_in')
  ...
  where i.company_id=p_company_id
    ...
    and i.repair_id is null
    and i.status<>'void'
    and i.issued_at>=v_start and i.issued_at<v_end;
  ...
  where f.company_id=p_company_id and f.occurred_at>=v_start and f.occurred_at<v_end;

  v_cash_inflows:=v_cash;

  select coalesce(sum(balance),0) into v_customer_open
  from (
    select greatest(coalesce(sum(d.debit),0)-coalesce(sum(d.credit),0),0) balance
    from public.customer_debt_ledger d
    where d.company_id=p_company_id and d.created_at<v_start
    group by d.customer_id
  ) x;

  select coalesce(sum(d.debit),0),coalesce(sum(d.credit),0)
  into v_customer_charges,v_customer_payments
  from public.customer_debt_ledger d
  where d.company_id=p_company_id and d.created_at>=v_start and d.created_at<v_end;

  select coalesce(sum(balance),0) into v_customer_close
  from (
    select greatest(coalesce(sum(d.debit),0)-coalesce(sum(d.credit),0),0) balance
    from public.customer_debt_ledger d
    where d.company_id=p_company_id and d.created_at<v_end
    group by d.customer_id
  ) x;

  select coalesce(sum(balance),0) into v_engineer_open
  from (
    select greatest(
      coalesce(sum(t.debit) filter(where t.transaction_type<>'payment_out'),0)
      -coalesce(sum(t.credit),0),0
    ) balance
    from public.engineer_transactions t
    where t.company_id=p_company_id and t.created_at<v_start
  ...
  from public.engineer_transactions t
  where t.company_id=p_company_id and t.created_at>=v_start and t.created_at<v_end;

  select coalesce(sum(balance),0) into v_engineer_close
  from (
    select greatest(
      coalesce(sum(t.debit) filter(where t.transaction_type<>'payment_out'),0)
      -coalesce(sum(t.credit),0),0
    ...
    from public.engineer_transactions t
    where t.company_id=p_company_id and t.created_at<v_end
    group by t.engineer_id
  ) x;

  select
    coalesce(sum(m.total_cost) filter(where m.movement_type='sale'),0),
  ...
  where f.company_id=p_company_id
    and f.direction='out'
    and f.category in('salary','rent','utility','other')
    and f.occurred_at>=v_start and f.occurred_at<v_end;

  select coalesce(sum(f.amount),0) into v_engineer_direct_cost
  from public.financial_transactions f
  where f.company_id=p_company_id
    and f.direction='out'
    and f.category='engineer_payment'
    and f.occurred_at>=v_start and f.occurred_at<v_end;

  select coalesce(sum(f.amount),0) into v_customer_refunds
  from public.financial_transactions f
  where f.company_id=p_company_id
    and f.direction='out'
    and f.category='customer_refund'
    and f.occurred_at>=v_start and f.occurred_at<v_end;
  ...
  v_net_profit:=v_gross_profit-v_customer_refunds-v_operating_expenses-v_engineer_direct_cost;

  return query
  select
    p_company_id,p_business_date,v_timezone,p_opening_cash,(p_opening_cash is not null),
