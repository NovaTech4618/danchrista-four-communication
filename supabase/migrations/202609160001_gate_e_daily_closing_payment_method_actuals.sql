...
begin
  ...
  foreach v_method in array array['cash','transfer','pos','other'] loop
    if not (p_payment_methods ? v_method) then
      raise exception 'Actual amount is required for %', v_method;
    end if;
    begin
      v_actual := (p_payment_methods ->> v_method)::numeric;
    exception when invalid_text_representation then
      raise exception 'Actual amount for % must be a valid number', v_method;
    end;
    if v_actual is null or v_actual < 0 then
      raise exception 'Actual amount for % must be zero or greater', v_method;
    end if;
  end loop;

  select * into v_row
  from public.close_daily_closing(p_daily_closing_id, p_actual_cash, p_notes);

  foreach v_method in array array['cash','transfer','pos','other'] loop
    v_actual := (p_payment_methods ->> v_method)::numeric;
    update public.daily_closing_payment_methods
    set actual_amount = round(v_actual,2), updated_at = now()
