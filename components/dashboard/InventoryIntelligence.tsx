...
export default function InventoryIntelligence() {
  ...
  if (loading) return <div className="rounded-2xl border border-[#dfe6df] bg-white p-6 text-sm text-[#74837e]">Reading stock movement…</div>;

  return <section className="rounded-2xl border border-[#dfe6df] bg-white shadow-[0_10px_28px_rgba(18,59,52,0.06)]">
    <div className="flex flex-col gap-3 border-b border-[#edf0ed] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
      <div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#1d6a54]">Inventory intelligence</p><h2 className="mt-1 font-heading text-lg font-bold text-[#182a28]">What should the owner know about stock?</h2></div>
      <Link href="/inventory" className="inline-flex items-center gap-1 text-xs font-bold text-[#1d6a54]">Open inventory <ArrowRight className="size-3" /></Link>
    </div>
    ...
    <div className="flex items-center gap-2 border-t border-[#edf0ed] px-5 py-3 text-xs text-[#74837e]"><PackageSearch className="size-3.5" />Movement is calculated from recorded sales; it does not change stock by itself.</div>
  </section>;
}
...
function Insight({ title, icon: Icon, tone, count, children }: { title: string; icon: typeof AlertTriangle; tone: "amber" | "green" | "slate"; count: number; children: React.ReactNode }) {
  const tones = { amber: "bg-[#fff8e9] text-[#8a641d]", green: "bg-[#eef4f1] text-[#1d6a54]", slate: "bg-[#f4f6f5] text-[#53635e]" };
  return <div className="rounded-xl border border-[#edf0ed] p-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className={`flex size-8 items-center justify-center rounded-lg ${tones[tone]}`}><Icon className="size-4" /></span><h3 className="text-sm font-bold text-[#182a28]">{title}</h3></div><span className="rounded-full bg-[#f4f6f5] px-2 py-1 text-[11px] font-bold text-[#687974]">{count}</span></div><div className="mt-3 space-y-2">{children}</div></div>;
}
function Row({ name, detail }: { name: string; detail: string }) { return <div className="rounded-lg bg-[#f8faf8] px-3 py-2"><p className="truncate text-xs font-semibold text-[#263734]">{name}</p><p className="mt-0.5 text-[11px] text-[#74837e]">{detail}</p></div>; }
function Empty({ children }: { children: React.ReactNode }) { return <p className="py-3 text-xs leading-5 text-[#87948f]">{children}</p>; }
