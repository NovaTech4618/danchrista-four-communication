...
export default function InventoryForm({ editingItem, onSaved, onCancelEdit }: Props) {
  ...
  useEffect(() => {
    ...
    setSellingPrice(String(editingItem.selling_price)); setCostPrice(editingItem.cost_price != null ? String(editingItem.cost_price) : ""); setQuantity(String(editingItem.quantity)); setMinimumStock(String(editingItem.minimum_stock));
  ...
  function resetForm() {
    setItemName(""); setCategory("Phone Parts"); setSubcategory("Other Phone Parts"); setItemType("part"); setBrand(""); setCompatibleModels(""); setSku(""); setSellingPrice(""); setCostPrice(""); setQuantity("0"); setMinimumStock("5"); setSupplier(""); setShelfLocation(""); setNotes(""); setImageUrl(null); setImageFile(null); setRemoveImage(false);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    const payload = { item_name: itemName.trim(), category, subcategory, item_type: itemType, brand: brand.trim() || null, compatible_models: compatibleModels.trim() || null, sku: sku.trim() || null, selling_price: sp, cost_price: cp, quantity: q, minimum_stock: m, supplier: supplier.trim() || null, shelf_location: shelfLocation.trim() || null, notes: notes.trim() || null, image_url: null };
