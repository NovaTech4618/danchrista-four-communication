...
export default function InventoryForm({ editingItem, onSaved, onCancelEdit }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (category === "Phone Parts" && subcategory === "Back Glass / Housing" && brand.trim().toLowerCase() !== "iphone") {
      return toast.error("Back Glass / Housing is currently for iPhone stock only.");
    }
  ...
  return (
    <Card className="h-fit border-slate-200 shadow-sm">
      ...
      <CardContent><form onSubmit={handleSubmit} className="space-y-4">
        ...
        {category === "Phone Parts" && <datalist id="phone-brands">
          {["iPhone", "Samsung", "Tecno", "Infinix", "itel", "Redmi", "Xiaomi", "Nokia", "Huawei", "Oppo", "Vivo", "Other Android"].map(value => <option key={value} value={value} />)}
        </datalist>}
        <Input placeholder={category === "Phone Parts" ? "Compatible model(s) — e.g. Galaxy A15, iPhone 13" : "Compatible model / details (optional)"} value={compatibleModels} onChange={e => setCompatibleModels(e.target.value)} />
