...
const PART_SUBCATEGORIES = ["Charging Boards", "Charging Flex", "Power Button Flex", "Earpiece / Speaker Flex", "Back Glass", "Other Phone Parts"];
const GOODS_SUBCATEGORIES = ["Chargers", "Cables", "Earphones", "Headsets", "Power Banks", "Speakers", "Screen Protectors", "Other Accessories"];
...
export default function InventoryForm({ editingItem, onSaved, onCancelEdit }: Props) {
  ...
  useEffect(() => {
    ...
    setSubcategory((editingItem.subcategory === "Downboards" ? "Charging Boards" : editingItem.subcategory === "Power Flex" ? "Power Button Flex" : editingItem.subcategory === "Earpiece Flex" ? "Earpiece / Speaker Flex" : editingItem.subcategory) || (normalizedCategory === "Phone Parts" ? "Other Phone Parts" : "Other Gadgets & Accessories"));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (cp !== null && (!Number.isFinite(cp) || cp < 0)) return toast.error("What the shop paid cannot be negative.");
  ...
  return (
    <Card className="h-fit border-slate-200 shadow-sm">
      ...
      <CardContent><form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><label className="space-y-1"><span className="text-xs font-semibold text-slate-600">Normal selling price</span><Input type="number" min="0.01" step="0.01" placeholder="Selling price" value={sellingPrice} onChange={e => setSellingPrice(e.target.value)} required /></label><label className="space-y-1"><span className="text-xs font-semibold text-slate-600">Lowest price staff can sell</span><Input type="number" min="0" step="0.01" placeholder="₦" value={minimumSellingPrice} onChange={e => setMinimumSellingPrice(e.target.value)} required /><span className="text-[11px] text-slate-500">Going below this needs Boss approval.</span></label><label className="space-y-1"><span className="text-xs font-semibold text-slate-600">What the shop paid</span><Input type="number" min="0" step="0.01" placeholder="Cost price" value={costPrice} onChange={e => setCostPrice(e.target.value)} /></label></div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Input type="number" min="0" step="1" placeholder="Quantity" value={quantity} onChange={e => setQuantity(e.target.value)} required /><Input type="number" min="0" step="1" placeholder="Warn me when it reaches" value={minimumStock} onChange={e => setMinimumStock(e.target.value)} required /></div>
