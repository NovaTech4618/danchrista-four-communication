...
type DanchristaLogoProps = { dark?: boolean; compact?: boolean; className?: string };
    ...
    <span className={`inline-flex items-center gap-3 ${className}`}>
      ...
      {!compact && (
        <span className="leading-none">
          ...
          <span className={`mt-1.5 block text-[8px] font-bold uppercase tracking-[0.22em] ${dark ? "text-slate-400" : "text-slate-400"}`}>Amezing Limited</span>
        </span>
      )}
    </span>
  );
