...
import { BarChart3, Boxes, ClipboardList, HandCoins, HelpCircle, LayoutDashboard, LogOut, MessageCircle, Package, Search, Settings, ShoppingCart, UserCog, Users, WalletCards, Wrench, ArrowDownToLine, ArrowUpFromLine } from "lucide-react";
...
const groups: NavGroup[] = [
  ...
  { label: "Money owed", items: [
    { title: "Debit / Owed to us", url: "/outstanding", icon: HandCoins, permission: "outstanding" },
    { title: "Credit / We owe", url: "/finance", icon: WalletCards, permission: "payments" },
    { title: "Engineer records", url: "/engineer-workflow", icon: UserCog, permission: "engineer_work" },
  ] },
  { label: "Owner view", items: [
    ...
    { title: "Low stock & alerts", url: "/alerts", icon: Boxes, permission: "inventory" },
  ] },
...
export default function AppSidebar() {
  ...
  return <Sidebar collapsible="icon" className="border-r border-[#d7a95a]/10 bg-[#123b34] text-white shadow-[12px_0_40px_rgba(18,59,52,0.14)]">
    ...
    <SidebarContent className="bg-[#123b34] px-2 py-3">
      ...
      {hasPermission(myRole, "settings") && <SidebarGroup className="px-0 pt-2"><SidebarGroupLabel className="px-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#9eb8ad]">System</SidebarGroupLabel><SidebarGroupContent><SidebarMenu><SidebarMenuItem><SidebarMenuButton render={<a href="/settings" />} className={menuButtonClass(pathname.startsWith("/settings"))} tooltip="Settings"><Settings className="size-4" /><span>Settings</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarGroupContent></SidebarGroup>}
    </SidebarContent>
