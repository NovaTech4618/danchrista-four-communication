...
export default function AppSidebar() {
  ...
  useEffect(() => { let active = true; void staffService.getMyRole().then(({ data }) => { if (active && data) setMyRole(data); }); void companyService.getCompany().then(({ data }) => { if (!active || !data) return; const record = data as unknown as { full_name?: string; companies?: { name?: string } | { name?: string }[] }; const companyRecord = Array.isArray(record.companies) ? record.companies[0] : record.companies; setProfile({ fullName: record.full_name ?? "Account", companyName: companyRecord?.name ?? "Danchrista Four Communication" }); }); return () => { active = false; }; }, []);
  async function handleLogout() { await supabase.auth.signOut(); router.push("/login"); }
  function renderItem(item: NavItem) { if (!hasPermission(myRole, item.permission)) return null; const active = pathname === item.url || (item.url !== "/dashboard" && pathname.startsWith(`${item.url}/`)); return <SidebarMenuItem key={item.url}><SidebarMenuButton isActive={active} tooltip={item.title} render={<a href={item.url} />} className={menuButtonClass()}><item.icon className="size-[17px]" aria-hidden="true" /><span>{item.title}</span></SidebarMenuButton></SidebarMenuItem>; }
  function renderGroup(group: NavGroup) { const allowedItems = group.items.filter((item) => hasPermission(myRole, item.permission)); if (!allowedItems.length) return null; return <SidebarGroup key={group.label} className="px-1.5 py-2"><SidebarGroupLabel className="px-3 pb-1 text-[10px] font-bold uppercase tracking-[0.17em] text-slate-600 group-data-[collapsible=icon]:px-0">{group.label}</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>{allowedItems.map(renderItem)}</SidebarMenu></SidebarGroupContent></SidebarGroup>; }
  const canManageStaff = hasPermission(myRole, "staff"); const initial = profile?.fullName?.trim()?.[0]?.toUpperCase() ?? "D";
  const management: NavItem[] = [ ...(canManageStaff ? [{ title: "Staff", url: "/staff", icon: Users, permission: "staff" as const }] : []), ...(hasPermission(myRole, "search") ? [{ title: "Search", url: "/search", icon: Search, permission: "search" as const }] : []), ...(hasPermission(myRole, "settings") ? [{ title: "Settings", url: "/settings", icon: Settings, permission: "settings" as const }] : []) ];
  return <Sidebar collapsible="icon" className="border-r border-white/[0.07] bg-[#111111] text-slate-200"><SidebarHeader className="border-b border-white/[0.06] bg-[#111111] px-3 py-3"><div className="flex items-center gap-3 px-1 py-1"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#12b76a] font-heading text-sm font-black text-white">D</div><div className="min-w-0 group-data-[collapsible=icon]:hidden"><span className="font-heading text-base font-bold tracking-tight text-white">Danchrista</span><span className="block text-[10px] font-medium uppercase tracking-[0.14em] text-slate-500">Four Communication</span></div></div></SidebarHeader><SidebarContent className="bg-[#111111] px-2 py-2"><SidebarGroup className="px-1.5 py-2"><SidebarMenu>{hasPermission(myRole, "dashboard") && <SidebarMenuItem><SidebarMenuButton isActive={pathname === "/dashboard"} tooltip="Dashboard" render={<a href="/dashboard" />} className={menuButtonClass()}><LayoutDashboard className="size-[17px]" aria-hidden="true" /><span>Dashboard</span></SidebarMenuButton></SidebarMenuItem>}</SidebarMenu></SidebarGroup>{renderGroup(operations)}{renderGroup(business)}{renderGroup(money)}{renderGroup(communication)}{management.length > 0 && renderGroup({ label: "Manage", items: management })}</SidebarContent><SidebarFooter className="border-t border-white/[0.06] bg-[#111111] p-2"><div className="mb-2 flex items-center gap-3 rounded-lg bg-white/[0.04] px-3 py-2.5 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#12b76a] text-sm font-black text-white">{initial}</div><div className="min-w-0 group-data-[collapsible=icon]:hidden"><p className="truncate text-sm font-semibold text-white">{profile?.fullName ?? "Loading…"}</p><p className="truncate text-xs text-slate-500">{profile?.companyName ?? "Danchrista Four Communication"}</p></div></div><SidebarMenu><SidebarMenuItem><SidebarMenuButton tooltip="Help & Support" render={<a href="/help" />} className={menuButtonClass()}><HelpCircle className="size-[17px]" aria-hidden="true" /><span>Help & Support</span></SidebarMenuButton></SidebarMenuItem><SidebarMenuItem><SidebarMenuButton tooltip="Log out" onClick={handleLogout} className={menuButtonClass()}><LogOut className="size-[17px]" aria-hidden="true" /><span>Log out</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarFooter></Sidebar>;
}
