...
export default function AppSidebar() {
  ...
  function renderItem(item: NavItem) { if (!hasPermission(myRole, item.permission)) return null; const active = pathname === item.url || pathname.startsWith(`${item.url}/`); return <SidebarMenuItem key={item.title}><SidebarMenuButton type="button" isActive={active} onClick={() => router.push(item.url)} className={menuButtonClass(active)} tooltip={item.title}><item.icon className="size-4" /><span>{item.title}</span></SidebarMenuButton></SidebarMenuItem>; }
  return <Sidebar collapsible="icon" className="border-r border-[#d7a95a]/10 bg-[#123b34] text-white shadow-[12px_0_40px_rgba(18,59,52,0.14)]">
    ...
    <SidebarContent className="bg-[#123b34] px-2 py-3">
      <SidebarMenu className="mb-2"><SidebarMenuItem><SidebarMenuButton type="button" isActive={pathname === "/dashboard"} onClick={() => router.push("/dashboard")} className={menuButtonClass(pathname === "/dashboard")} tooltip="Dashboard"><LayoutDashboard className="size-4" /><span>Dashboard</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu>
      ...
      {hasPermission(myRole, "settings") && <SidebarGroup className="px-0 pt-2"><SidebarGroupLabel className="px-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#9eb8ad]">System</SidebarGroupLabel><SidebarGroupContent><SidebarMenu><SidebarMenuItem><SidebarMenuButton type="button" isActive={pathname.startsWith("/settings")} onClick={() => router.push("/settings")} className={menuButtonClass(pathname.startsWith("/settings"))} tooltip="Settings"><Settings className="size-4" /><span>Settings</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarGroupContent></SidebarGroup>}
    </SidebarContent>
