...
function menuButtonClass(active: boolean) { return `h-10 rounded-xl border transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7FE7D6]/70 ${active ? "border-white/10 bg-white text-[#0B3D91] shadow-sm" : "border-transparent text-[#DCEEFF] hover:border-white/5 hover:bg-white/10 hover:text-white"}`; }
...
export default function AppSidebar() {
  ...
  return <Sidebar collapsible="icon" className="border-r border-[#7FE7D6]/10 bg-[#0B3D91] text-white shadow-[12px_0_40px_rgba(18,59,52,0.14)]">
    <SidebarHeader className="border-b border-white/10 bg-[#0B3D91] px-3 py-4"><a href="/dashboard" className="block rounded-xl px-1 py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7FE7D6]"><DanchristaLogo dark /></a></SidebarHeader>
    <SidebarContent className="bg-[#0B3D91] px-2 py-3">
      ...
      {groups.map((group) => { const allowed = group.items.filter((item) => hasPermission(myRole, item.permission)); if (!allowed.length) return null; return <SidebarGroup key={group.label} className="px-0"><SidebarGroupLabel className="px-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#A9C8E5]">{group.label}</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>{allowed.map(renderItem)}</SidebarMenu></SidebarGroupContent></SidebarGroup>; })}
      {hasPermission(myRole, "settings") && <SidebarGroup className="px-0 pt-2"><SidebarGroupLabel className="px-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#A9C8E5]">System</SidebarGroupLabel><SidebarGroupContent><SidebarMenu><SidebarMenuItem><SidebarMenuButton render={<a href="/settings" />} isActive={pathname.startsWith("/settings")} className={menuButtonClass(pathname.startsWith("/settings"))} tooltip="Settings"><Settings className="size-4" /><span>Settings</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarGroupContent></SidebarGroup>}
    </SidebarContent>
    <SidebarFooter className="border-t border-white/10 bg-[#0B3D91] p-3"><div className="mb-2 hidden rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 group-data-[collapsible=icon]:hidden"><p className="truncate text-xs font-semibold text-white">{profile?.fullName || "Staff member"}</p><p className="mt-0.5 truncate text-[10px] text-[#A9C8E5]">{profile?.companyName || "Danchrista Four Communication"}</p></div><button type="button" onClick={handleLogout} className="flex w-full items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm font-medium text-[#DCEEFF] transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7FE7D6]"><LogOut className="size-4 text-[#7FE7D6]" /><span>Log out</span></button></SidebarFooter>
  </Sidebar>;
}
