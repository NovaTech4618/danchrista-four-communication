...
export default function RepairInvoiceWorkflow({ repairId, customerId, repairTotal }: Props) {
  ...
  async function load() {
    setLoading(true);
    const { data, error } = await businessOperationsService.getRepairInvoice(repairId);
    setLoading(false);
  ...
  async function issueInvoice() {
    ...
    if (!Number.isFinite(repairTotal) || repairTotal <= 0) return toast.error("Set a valid repair cost before issuing the invoice.");
    setBusy(true);
    const invoiceNumber = `INV-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
    const result = await businessOperationsService.createRepairInvoice({ repairId, invoiceNumber, amount: repairTotal });
    setBusy(false);
