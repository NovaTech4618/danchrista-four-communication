...
type ProfitRow = { revenue?: number | null; amount_paid?: number | null; outstanding?: number | null };
...
export default function RepairDeskWorkflow({ repairId, status, engineerAssigned, hasHandover }: Props) {
  ...
  if (paid > 0 && outstanding <= 0) completed.add(5);
  ...
  else if (outstanding > 0) next = "Collect the outstanding balance";
  ...
  return <Card className="border-slate-200 bg-slate-950 text-white shadow-sm">
    <CardContent className="p-4 sm:p-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-teal-300">Repair desk · next action</p><p className="mt-1 text-base font-semibold">{next}</p></div><div className="text-xs text-slate-400">Paid <span className="font-semibold text-white">₦{paid.toLocaleString("en-NG")}</span> · Outstanding <span className="font-semibold text-white">₦{outstanding.toLocaleString("en-NG")}</span></div></div>
